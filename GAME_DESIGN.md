...
`peixes_por_clique` é o **valor-base**: a proteção de cadência aplicada depois pode reduzi-lo abaixo
de 1 durante uma rajada sobre-humana. Isso preserva o piso de bootstrap sem criar um mínimo
explorável por autoclicker.
...
> sem travar; nº de dinastias sintoniza pelo DIVISOR. Números → §8.
>
> **✅ IMPLEMENTADO (2026-07-16):** `domain/legendaries.ts` (puro: buff por nível, combinação,
> custos, sorteio) + `data/legendaries.ts` (elenco de 13, 3 tiers, constantes §8) + store (recrutar/
> reroll/subir nível, Lendários na produção/clique/custo/offline/lump, Selo→#0) + save **v3**
> (migração v2→v3 converte o Selo) + painel de UI (`.modal-corte`). `CROWN_BONUS` removido, Coroa em
> `cbrt`. tsc + 105 testes (13 domain + 2 de conteúdo + Corte na store + migração no save) + build verdes.
...
| 2026-07-16 | **✅ v0.7 — Corte Lendária IMPLEMENTADA (ADR-0004)** | Motor endless ligado. `domain/legendaries.ts` (puro: `buffNoNivel` composto, `multiplicadoresLendarios` por eixo, custos recrutar/nível/reroll, `sortearOferta` com rng injetado) + `data/legendaries.ts` (elenco de 12: 1 grátis #0 + 11 em tiers 1/3/5; papéis produção/clique/custo/offline/lump; constantes §8). Store: **Coroa gastável**, ações `recrutarLendario`/`rerollOferta`/`subirNivelLendario`, draft de 3 com pool por `eraMaxAtingida` (permanente), Lendários entram em produção/clique/custo-de-gatos/offline/lump; **`CROWN_BONUS` removido** e produção vira só `lendarios×evento`; prestígio `sqrt`→**`cbrt`** (DIVISOR 1e6→1e4); **Selo→Lendário #0** (some a flag `seloImperial`). Save **v3** (migração v1→v2→v3; `seloImperial:true`→`{selo_imperial:1}`). Painel de UI (`.modal-corte`: recrutados com subir-nível, oferta com recrutar, reroll). **Removidos:** `CROWN_BONUS`, `SELO_IMPERIAL_MULT`, `bonusGlobalDeCoroas`, `seloImperial`. tsc + **103 testes** (+13 domain, +Corte na store, +migração v3) + build verdes. Números finos = §8 (a *forma* foi validada por simulação). Pendente: arte dos Lendários (emoji placeholder); playtest de balanceamento |
...
| 2026-07-16 | **Princesa Helena — nova Gata Lendária** | Homenagem adicionada ao tier 5 da Corte: filhote-princesa da linhagem visual da Imperatriz Jady, com papel próprio de virada de Era (`lumpMult ×1,75/nível`). Arte bespoke 1024×1024 com alpha, registrada em `ui/legendaryArt.ts`; elenco passa a 13 Lendários. |
| 2026-07-16 | **Latão Gourmet — conjunto visual próprio** | O 5º prédio deixa de reutilizar a Caixa: novo ícone de prédio, novo gato trabalhador `catador` e nova lane detalhada, todos na identidade carvão + vinho + latão envelhecido e registrados em `ui/buildingArt.ts`. Os prédios posteriores continuam no fallback até receberem seus conjuntos. |
| 2026-07-16 | **Gato de Schrödinger — arte própria** | O Lendário de ganho offline deixa o emoji-placeholder: gato em superposição visual carvão/prata, olhos ciano/violeta, caixa quântica e relógio de bolso, em PNG 1024×1024 com alpha e registrado em `ui/legendaryArt.ts`. |
| 2026-07-16 | **Guardião do Vórtex — arte própria** | O Lendário galáctico de ganho offline recebe arte: pelagem roxa com nebulosas e constelações integradas, halo orbital, coroa de vórtex e ampulheta cósmica. PNG 1024×1024 com alpha, registrado em `ui/legendaryArt.ts`. |
| 2026-07-16 | **Barão von Bigode — arte própria** | O Lendário aristogato de produção global deixa o emoji-placeholder: gato tigrado ruivo com bigode creme, cartola, monóculo, casaca vinho, bengala de peixe e livro-caixa. PNG 1024×1024 com alpha, registrado em `ui/legendaryArt.ts`. |
| 2026-07-16 | **Maré de Peixe — 1ª Habilidade ativa implementada (§3.5)** | Primeiro slice do eixo ativo: Barraca de Peixe com ≥1 gato habilita **Maré de Peixe** (clique ×5 por 15 s; recarga de 90 s). A regra temporal é pura e testada; o estado preserva a recarga no save, mas descarta a janela de burst ao voltar do offline. A lane mostra disparo/ativa/recarga e o peixe recebe feedback turquesa durante o pico. Proteção anti-autoclicker: até 8 cliques/s rendem integralmente; acima disso, cada clique vale 80% do anterior, em uma curva saturante de teto prático ≈12 cliques integrais/s. Testes: domínio, store e save. Festival e conquistas continuam como próximos sistemas. |
| 2026-07-16 | **Feedback de clique e MarÃ© de Peixe â€” correÃ§Ãµes de playtest** | Ativas renderizadas na coluna sticky do peixe (acompanham o scroll); HUD e partÃ­cula usam o ganho efetivamente creditado; passivas C1 e MarÃ© multiplicam a base mesmo durante o piso de bootstrap; lifetime recebe contraste creme. CadÃªncia continua aplicando reduÃ§Ã£o apenas acima de 8 cliques/s. |
